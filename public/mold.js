// Functions for mold management with multiplayer support
...
function spawnMold() {
    ...
    while (attempts < 100) {
        ...
        if (grid[y][x] !== TERRAIN.MOLD) {
            ...
            const newMold = {x, y, size: 1, growthRate: 1};
            moldSpots.push(newMold);
            
            // Notify other clients about the new mold spot
            if (isConnected) {
                sendNewMold(x, y, 1, 1);
            }
        ...
        for (let g = 0; g < growthFactor; g++) {
            if (Math.random() < 0.3 * difficulty) {
                ...
                if (randomDir && grid[randomDir.y][randomDir.x] !== TERRAIN.MOLD) {
                    ...
                    // rather than sending individual updates for each new mold cell
                    if (isConnected && isHost) {
                        sendNewMold(randomDir.x, randomDir.y, 1, spot.growthRate * 1.1);
                    }
                }
            }
        }
