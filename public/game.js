// Main game logic and initialization with multiplayer support
...
let isHost = false;         // Whether this client is the host (responsible for spawning mold)
...
function initGame() {
    ...
    // Determine if this client is the host after a short delay
    // Host is responsible for spawning mold for all clients
    setTimeout(determineHost, 2000);
}

// Determine if this client should be the host
function determineHost() {
    // If player ID is the lowest of all connected players, become the host
    // This is a simple way to elect a consistent host
    if (playerId && socket && socket.readyState === WebSocket.OPEN) {
        // For now, player 1 is always host
        // In a more advanced implementation, this would check all player IDs
        isHost = playerId.endsWith('1') || playerId.startsWith('1');
...
function handleCanvasClick(event) {
    ...
    const removed = removeMold(cellX, cellY);
    
    // If successful and connected to network, notify other players
    if (removed) {
        sendMoldRemoval(cellX, cellY);
    }
}
...
function gameLoop() {
    ...
    if (isHost) {
        ...
        if (currentTime - lastMoldGrowth > moldGrowthRate) {
            ...
            // Notify other clients about mold growth
            sendMoldGrowth();
        }
    }
